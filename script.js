...
const loadCourses = async () => {
  try {
    ...
    populateCountryOptions(courses);
    renderCourses();
  } catch (error) {
...
const populateCountryOptions = (data) => {
  const countries = Array.from(new Set(data.map((course) => course.country)))
    .filter(Boolean)
    .sort();
...
const getFilteredCourses = () => {
  ...
  const countryQuery = countrySearch.value.trim().toLowerCase();
  ...
  let filtered = courses.filter((course) => {
    ...
    const matchesCountry = countryQuery
      ? course.country.toLowerCase().includes(countryQuery)
      : true;
      ...
      return `
        <article class="course-card">
          ...
          <div class="course-meta">
            ...
            <span><strong>Country</strong><span>${course.country}</span></span>
          </div>
...
[universitySearch, courseAreaSearch, countrySearch].forEach((input) => {
